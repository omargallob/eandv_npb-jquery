$(document).ready(function(){
	...
	// get the file name (ex: myPage.html, myPage.php, etc.) used to switch variation recipe below
	var jVariationsPage = location.href.split('/');jVariationsPage = jVariationsPage[jVariationsPage.length-1];

	// if jVariation does not find any elements, no checkbox/radio button will be written... but...
	// when including jVariations on several pages it is helpful to switch recipes based on the page being viewed
	// this is not required for jVariations to work, but it helps keep things organized :)
	switch(jVariationsPage){

		case 'jquery.variations.html':
			...
			// checkbox
			$('pre:lt(3)').jVariation({
				label: 'Show the hidden code',
				autoHide: true
			});

			break;

	} // close switch(jVariationsPage)
